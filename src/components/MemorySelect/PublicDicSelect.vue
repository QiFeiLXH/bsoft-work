...
<script>
import { getPubDicList } from '@/api/common'

export default {
  name: 'PublicDicSelect',
  props: {
    type: {
      type: Number,
      default: 30
  ...
  methods: {
    ...
    handleSelect (value, option) {
      const historyList = localStorage.getItem(this.storageKey) == null ? [] : JSON.parse(localStorage.getItem(this.storageKey))
      const data = { ...this.list.find(x => x.id === value) }
      data['useful'] = 1
      ...
      const record = this.list.find(item => item.id === value)
